...
#NUM_SAMPLES =
DIR = os.getcwd()
#MIN_RESOURCE = 1 #till this no of epoch the pruning will not happen for the current fold
...
def plot_training_validation_loss(epochs, train_losses, val_losses):
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig("Results/GRID/No_pruner/1_dec/noP.png")
...
def run_optuna():
    ...
    total_trials = len(sampler._all_grids)
    print(f"Total number of trials: {total_trials}")
    study = optuna.create_study(
        storage="sqlite:///grid_search_Nopruner_1_dec.db",
        study_name="GridSearch_NoPruner_1_dec",
    ...
    fig1.write_image("Images/GRID/No_pruner/optimization_history_1_dec.png")


    fig2 = optuna.visualization.plot_timeline(study)
    #fig2.show()

    fig2.write_image("Images/GRID/No_pruner/timeline_1_dec_.png")
