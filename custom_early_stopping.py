...
'''def load_data():
    ...
    trainset = datasets.MNIST(root=DIR, train=True, download=True, transform=transform)
    testset = datasets.MNIST(root=DIR, train=False, download=True, transform=transform)
...
def load_data():
    ...
    with suppress_stdout():
        trainset = datasets.MNIST(root="./data", train=True, download=True,
                                  transform=transform)  # You can keep this default to ensure data is downloaded to a known location
...
def train_mnist(trial, config, train_loader, val_loader, max_num_epochs):
    ...
    for epoch in range(max_num_epochs):
        ...
        if epoch >= min_resrc:
            # Calculate relative improvement in validation loss
            # relative_improvement = (best_cmp - val_loss) / best_cmp

            if val_loss < best_cmp:
                relative_improvement = (best_cmp - val_loss)
...
def plot_training_validation_loss(epochs, train_losses, val_losses):
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig("Results/TPE/custom_pruner/custom_pruner_24T.png")
...
def objective(trial):
    ...
    print(
        f"Mean validation loss across all folds for Trial {trial.number + 1} is {mean_validation_loss:.4f} with trial config:  l1: {config['l1']}, l2: {config['l2']}, lr: {config['lr']}, batch_size: {config['batch_size']}")
...
def run_optuna(num_samples=NUM_SAMPLES):
    ...
    study = optuna.create_study(
        storage="sqlite:///custompruner_10_dec.db",
        study_name="custompruner_10_dec_nov",
    ...
    fig1.write_image("Results/TPE/custom_pruner/optimization_history_10_dec_custom.png")

    fig2 = optuna.visualization.plot_timeline(study)
    #fig2.show()

    fig2.write_image("Results/TPE/custom_pruner/timeline_10_dec_custom.png")
